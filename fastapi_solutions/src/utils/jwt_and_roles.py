...
async def validate_token(token: str) -> dict[str, str]:
    ...
    try:
        ...
        a_api_logger.error(f"Error while JWT decoding: {decode_error}")
        raise HTTPException(
            status_code=HTTPStatus.UNAUTHORIZED, detail="Неверный токен"
        )
    except jwt.ExpiredSignatureError:
        a_api_logger.error("Срок действия токена истек")
        raise HTTPException(
            status_code=HTTPStatus.UNAUTHORIZED, detail="Срок действия токена истек"
        )
    except ValueError as err:
        a_api_logger.error(f"Error while JWT decoding: {err}")
        raise HTTPException(
